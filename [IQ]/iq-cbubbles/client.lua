...
addEventHandler('onChatbubblesMessageIncome',root,function (msg,plr)
	addBubble(msg,plr,getTickCount())
end)

local font = exports['iq-fonts']:getFont('medium',8)
...
function addBubble(text, player, tick)
	...
	local _texture = nil
	if ROUND then
		local width = dxGetTextWidth(text:gsub('#%x%x%x%x%x%x', ''), 1, font)
...
function handleDrawMessage(message, index)
	...
	local yPos = outElastic(elapsedTime, message.yPos, (sy - 25*index) - message.yPos, ANIMATION_DURATION, 5)
	...
	if ROUND then
		dxDrawImage(finalContainerX, finalContainerY, finalContainerWidth, finalContainerHeight, message.texture, nil, nil, tocolor(31, 31, 31, message.alpha))
	else
		dxDrawRectangle(finalContainerX, finalContainerY, finalContainerWidth, finalContainerHeight, tocolor(0, 0, 0, message.alpha))
	end

	dxDrawText(message.text, finalContainerX, finalContainerY, finalContainerX + finalContainerWidth, finalContainerY + finalContainerHeight, tocolor(255, 255, 255, message.alpha+50), 1, font, 'center', 'center', false, false, false, true)
end
