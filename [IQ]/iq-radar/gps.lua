				...
				if localPlayer == plr and dim then
					...
					for i,row in ipairs(GPS.road) do
						if row.ID >= roadNum then
							if #GPS.road > 0 then
								exports["noobisty-notyfikacje"]:createNotification("GPS", "Dojechałeś na miejsce", {255, 50, 50}, "sight")
							end
							removeGPS()
						end
					end
...
function getPath(startNode, endNode)
	...
	while true do
		...
		if not bestNode then
			if getElementInterior(localPlayer) == 0 and getElementDimension(localPlayer) == 0 then
				exports["noobisty-notyfikacje"]:createNotification("GPS", "Nie można odnależć trasy", {255, 50, 50}, "sighter")
			end
...
-- findBestWay(-2253.51, 33.65, 35.17)