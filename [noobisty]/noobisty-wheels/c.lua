local settings = {
    maxDistance = 30,
    shaderDistance = 50,
    shader = "img/shader.fx",
    shaderOutter = "img/shaderOutter.fx",
}
...
function Wheels:constructor()
    ...
    self.textures = {}
    self.textures.wheelColor = dxCreateTexture("img/wounds.png", "dxt3", true, "clamp")
    self.textures.wounds = dxCreateTexture("img/wounds.png", "dxt3", true, "clamp")
...
function Wheels:calculateVehicleWheelRotation(vehicle, wheels)
    ...
    for i = 1, #wheels do
        ...
        if (wheel.object) then
            ...
            setElementDimension(wheel.object, getElementDimension(vehicle))
        end
    end
end
...
function Wheels:updateColor(veh)
    ...
    dxSetShaderValue(self.streamed[veh].vehicle, "sLightColor", Vector4(self.streamed[veh].color[7]/255, self.streamed[veh].color[8]/255, self.streamed[veh].color[9]/255, 1))
    engineApplyShaderToWorldTexture(self.streamed[veh].vehicle, "rims", veh)

    local data = getElementData(veh, "visualTuning")

    if not self.createdWheels[veh] then return end

    dxSetShaderValue(self.streamed[veh].wheel, "Brightness", 1.2)
    dxSetShaderValue(self.streamed[veh].wounds, "Brightness", 0.8)
    
    local model = getElementModel(self.createdWheels[veh][1].object)
    local wheels = {'wheel_lf_dummy', 'wheel_rf_dummy', 'wheel_rb_dummy', 'wheel_lb_dummy'}
    for i = 1, #wheels do
        dxSetShaderValue(self.streamed[veh].wheel, "innerSize", ((model == 1096) and 1 or data.innerSize))
        dxSetShaderValue(self.streamed[veh].wheel, "inner", (inners[model] or 0.45))
        dxSetShaderValue(self.streamed[veh].wheel, "sLightColor", Vector4(self.streamed[veh].color[7]/255, self.streamed[veh].color[8]/255, self.streamed[veh].color[9]/255, 1))
        engineApplyShaderToWorldTexture(self.streamed[veh].wheel, "rims", self.createdWheels[veh][i].object)
        setVehicleComponentVisible(veh, wheels[i], false)
    end

    for i = 1, #wheels do
        dxSetShaderValue(self.streamed[veh].wounds, "innerSize", ((model == 1096) and 1 or data.innerSize))
        dxSetShaderValue(self.streamed[veh].wounds, "inner", (inners[model] or 0.45))
        dxSetShaderValue(self.streamed[veh].wounds, "sLightColor", Vector4(self.streamed[veh].color[10]/255, self.streamed[veh].color[11]/255, self.streamed[veh].color[12]/255, 1))
        engineApplyShaderToWorldTexture(self.streamed[veh].wounds, "wounds", self.createdWheels[veh][i].object)
    end
    
    for i = 1, #wheels do
        dxSetShaderValue(self.streamed[veh].outter, "innerSize", ((model == 1096) and 1 or data.innerSize))
        dxSetShaderValue(self.streamed[veh].outter, "inner", (inners[model] or 0.45))
        engineApplyShaderToWorldTexture(self.streamed[veh].outter, "vehicletyres128", self.createdWheels[veh][i].object)
    end
end
...
function Wheels:onStreamIn(veh, details)
    ...
    if not self.streamed[veh] then
        self.streamed[veh] = {
            ...
            outter = dxCreateShader(settings.shaderOutter, 0, 0, false, "object"),
